...
#endif
{
    ...
    cf = std::unique_ptr<IIR_BandPass>(new IIR_BandPass(*so, 500, 200));
    ...
    addParameter(bandwidth = new AudioParameterFloat("bandwidth", // parameter ID
...
void WahWahAudioProcessor::processBlock(AudioBuffer<float> &buffer, MidiBuffer &midiMessages) {
    ...
    for (int i = 0; i < buffer.getNumSamples(); i++) {
        ...
        cf->set_bandwidth(bandwidth->get());
